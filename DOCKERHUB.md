...
Beyond chat, the proxy serves `/v1/embeddings`, `/v1/rerank` (Cohere-style document rerank), `/v1/images/generations`, `/v1/images/edits`, `/v1/images/variations`, `/v1/audio/speech` (TTS, binary or SSE), `/v1/audio/transcriptions`, and `/v1/audio/translations` (multipart STT) as transparent OpenAI-compatible pass-through. Only the `model` field is rewritten to the resolved upstream model; responses (JSON, SSE streams, binary audio) are forwarded verbatim. Every endpoint gets the same `hotel/` failover, virtual-key control, rate limiting, circuit breaker, and token metering as chat - and the same privacy guarantee: content (audio uploads, generated images, embedding inputs) is never inspected or logged. A native `POST /v1/messages` (Anthropic Messages API) brings `hotel/` failover to Claude Code and the anthropic SDKs: translation for non-Anthropic providers, native passthrough (thinking survives) for Anthropic; `x-api-key` or Bearer.
