...
GitHub is supported as a separate OAuth option. Being OAuth2 only (no ID token), it reads the account's verified emails from the GitHub API and matches them against the same fail-closed allowlist, logging the stable numeric id and login (source `github`). Register a GitHub OAuth App with callback URL `<public base URL>/api/auth/github/callback`, paste the Client ID and secret into Settings, and a "Sign in with GitHub" button appears. Redact `Location` on `/api/auth/github/callback` too if your proxy logs response headers.
