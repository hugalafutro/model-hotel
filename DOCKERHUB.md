...
SSO is a third login path, not a replacement: it mints the same session token as passkey and TOTP login. The allowlist fails closed, only verified emails match, the client secret is AES-256-GCM encrypted at rest, and the flow uses PKCE plus single-use state and nonce. The minted token is delivered in the URL fragment (never sent back to the server); if your reverse proxy logs response headers, redact `Location` on `/api/auth/oidc/callback`. Each operator registers their own OIDC app and points it at `<public base URL>/api/auth/oidc/callback`; the client must allow the `openid`, `email`, and `profile` scopes (all three are requested, so a client permitting fewer fails with `invalid_scope`), and the allowlist must hold the signing-in account's exact verified email. Local login (admin token, passkeys, TOTP) always keeps working, so a misconfigured provider cannot lock you out.
