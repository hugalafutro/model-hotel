...
func StdoutHandler() slog.Handler {
	...
	// Plain logfmt: a value holding a space is quoted and keeps its spaces,
	// which is what every log reader already expects and what makes a line
	// readable in docker logs. A caller-controlled value (a request path, a
	// key name, an error string) can therefore hold text shaped like a
	// "key=value" token of the binary's own; keeping a reader from acting on
	// that belongs to the reader, and the CrowdSec collection in
	// contrib/crowdsec documents how it does so. LOG_FORMAT=json gives that
	// pipeline discrete fields where the ambiguity cannot arise at all.
