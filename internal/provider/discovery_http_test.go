...
// Test GetDeepSeekBalance with mock server
func TestGetDeepSeekBalance(t *testing.T) {
	mockResponse := `{
		"cash_balance": 100.50,
		"bonus_balance": 50.25
	}`
	...
	ctx := context.Background()
	balance, err := svc.GetDeepSeekBalance(ctx, provider, masterKey)
	if err != nil {
...
// Test GetOpenRouterBalance with mock server
func TestGetOpenRouterBalance(t *testing.T) {
	creditsResponse := `{
		"data": {
	...
	keyResponse := `{
		"data": {
			"label": "test-key",
			"limit": 10000,
			"limit_reset": "2024-12-31T00:00:00Z",
			"limit_remaining": 8999.50,
			"usage": 1000.50,
			"usage_daily": 100.00,
			"usage_weekly": 500.00,
			"usage_monthly": 1000.00,
			"is_free_tier": false
		}
	}`
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch r.URL.Path {
		case "/credits":
			w.Write([]byte(creditsResponse))
		case "/key":
			w.Write([]byte(keyResponse))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	ctx := context.Background()
	balance, err := svc.GetOpenRouterBalance(ctx, provider, masterKey)
	if err != nil {
	...
	if callCount != 2 {
		t.Errorf("expected 2 API calls (credits + key), got %d", callCount)
	}
}
