...
func (h *TotpHandler) EnrollVerify(w http.ResponseWriter, r *http.Request) {
	...
	if h.useCookieAuth {
		...
		if h.sessionMgr != nil {
			if tok, err := h.sessionMgr.CreateAuthToken(r.Context(), []byte("admin"), nil, webauthn.MetaFromRequest(r)); err != nil {
...
func (h *TotpHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	sessionToken, err := h.sessionMgr.CreateAuthToken(r.Context(), []byte("admin"), nil, webauthn.MetaFromRequest(r))
	if err != nil {
