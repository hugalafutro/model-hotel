...
// TestCleanupLoop_TickerActuallyFires verifies that the cleanupLoop
...
func TestCleanupLoop_TickerActuallyFires(t *testing.T) {
	...
	// Insert a stale entry (last used 15 minutes ago)
	lim.mu.Lock()
	lim.limiters["ticker-fires-stale"] = &keyEntry{
		limiter:  rate.NewLimiter(10, 20),
		rps:      10,
		burst:    20,
		lastUsed: time.Now().Add(-15 * time.Minute),
	...
	// And a fresh entry
	lim.limiters["ticker-fires-fresh"] = &keyEntry{
		limiter:  rate.NewLimiter(10, 20),
		rps:      10,
		burst:    20,
		lastUsed: time.Now(),
	...
	// Start the cleanupLoop goroutine with the short ticker interval
	go lim.cleanupLoop()
	...
	lim.mu.Lock()
	_, hasStale := lim.limiters["ticker-fires-stale"]
	_, hasFresh := lim.limiters["ticker-fires-fresh"]
	lim.mu.Unlock()

	if hasStale {
		t.Error("stale key entry should have been removed by ticker-fired cleanup")
	}
	if !hasFresh {
		t.Error("fresh key entry should still be present after ticker-fired cleanup")
	}
