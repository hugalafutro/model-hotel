...
func TestBuildUpstreamBody(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := BuildUpstreamBody([]byte(inputBody), tt.providerType, tt.wantModel, "gpt-4", tt.isStreaming, cache, &sync.Map{}, tt.extraStrip, tt.providerType)
...
func TestBuildUpstreamBody_OpenCodeInjectsChatTemplateArgs(t *testing.T) {
	...
	result := BuildUpstreamBody([]byte(inputBody), "opencode-go", "glm-5.2", "glm-5.2", false, cache, &sync.Map{}, nil, "opencode-go")
...
func TestBuildUpstreamBody_LearnedRenameMovesValue(t *testing.T) {
	...
	result := BuildUpstreamBody([]byte(inputBody), "openai", "gpt-5-nano", "gpt-5-nano", false, &sync.Map{}, renameCache, nil, "openai")
...
func TestBuildUpstreamBody_UnparseableInput(t *testing.T) {
	...
	cache := &sync.Map{}
	result := BuildUpstreamBody([]byte("not json"), "openai", "gpt-4o", "gpt-4", true, cache, &sync.Map{}, nil, "openai")
...
func TestBuildUpstreamBody_StripsEmptyToolCalls(t *testing.T) {
	...
	result := BuildUpstreamBody([]byte(inputBody), "openai", "gpt-4o", "gpt-4", false, &sync.Map{}, &sync.Map{}, nil, "openai")
...
func TestBuildUpstreamBody_StripEmptyToolCallsTolerantOfShapes(t *testing.T) {
	...
	for _, body := range []string{
		...
		result := BuildUpstreamBody([]byte(body), "openai", "gpt-4", "gpt-4", false, &sync.Map{}, &sync.Map{}, nil, "openai")
		var raw map[string]any
		if err := json.Unmarshal(result, &raw); err != nil {
