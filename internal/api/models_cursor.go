...
func scanModelRow(rows pgx.Rows) (model.Model, error) {
	...
	err := rows.Scan(
		...
		&m.OwnedBy, &m.Enabled, &m.DisabledManually, &m.PriceCustomized, &m.PriceSources, &m.CreatedAt, &m.LastSeenAt, &m.ProviderName, &m.ProviderEnabled,
	)
	return m, err
}
