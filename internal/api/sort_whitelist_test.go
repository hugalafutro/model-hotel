...
func TestLogsSortDef_Whitelist(t *testing.T) {
	...
	allowedExprs := map[string]bool{
		...
		"CASE WHEN rl.tokens_prompt + rl.tokens_completion = 0 THEN CASE WHEN COALESCE(rl.error_message, '') ILIKE '%cancel%' OR COALESCE(rl.error_message, '') ILIKE '%disconnect%' OR COALESCE(rl.error_message, '') ILIKE '%context canceled%' THEN 1 ELSE 2 END ELSE 0 END": true,
		"rl.tokens_prompt + rl.tokens_completion":              true,
