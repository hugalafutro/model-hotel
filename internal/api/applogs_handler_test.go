...
// ---------------------------------------------------------------------------
// Pure function tests for applogs.go
// ---------------------------------------------------------------------------

func TestParseLogLine_AlreadyTested(t *testing.T) {
	// These are already tested in applogs_test.go, just verifying they exist
	source, level, msg := parseLogLine("2026/04/28 09:55:43 [proxy] INFO  message")
	if source != "proxy" || level != "info" || msg != "message" {
		t.Errorf("parseLogLine: got source=%q level=%q msg=%q, want source='proxy' level='info' msg='message'", source, level, msg)
	}
}

func TestStripLogTimestamp_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	result := stripLogTimestamp("2026/04/28 09:55:43 message")
	if result != "message" {
		t.Error("stripLogTimestamp should work as tested")
	}
}

func TestExtractSource_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	source, msg := extractSource("[proxy] message")
	if source != "proxy" || msg != "message" {
		t.Error("extractSource should work as tested")
	}
}

func TestDetectLevel_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	level := detectLevel("error occurred")
	if level != "error" {
		t.Error("detectLevel should work as tested")
	}
}

func TestWordMatch_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	if !wordMatch("error occurred", "error") {
		t.Error("wordMatch should work as tested")
	}
}

func TestIsWordChar_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	if !isWordChar('a') {
		t.Error("isWordChar should work as tested")
	}
}

func TestStripLevelPrefix_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	result := stripLevelPrefix("INFO  message")
	if result != "message" {
		t.Error("stripLevelPrefix should work as tested")
	}
}

func TestFilterEntriesAfter_AlreadyTested(t *testing.T) {
	// Already tested in applogs_test.go
	entries := []AppLogEntry{
		{Timestamp: "2026-01-01T10:00:00Z", Level: "info", Source: "a", Message: "first"},
		{Timestamp: "2026-01-01T11:00:00Z", Level: "info", Source: "b", Message: "second"},
	}
	result := filterEntriesAfter(entries, "2026-01-01T10:30:00Z")
	if len(result) != 1 {
		t.Error("filterEntriesAfter should work as tested")
	}
}
