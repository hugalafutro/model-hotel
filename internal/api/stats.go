...
func (h *StatsHandler) calculateStats(ctx context.Context, period time.Duration, excludeDeleted bool, metric string) (*StatsResponse, error) {
	...
	// Query 11: Avg TTFT
	query = `
		SELECT COALESCE(AVG(COALESCE(NULLIF(rl.ttft_ms, 0), rl.response_header_ms)) FILTER (WHERE COALESCE(NULLIF(rl.ttft_ms, 0), rl.response_header_ms) > 0), 0) as avg_ttft
		FROM request_logs rl` + vkJoin + `
		WHERE rl.created_at >= $1 AND rl.status_code > 0 AND rl.status_code < 400` + vkFilter
...
func (h *StatsHandler) GetTimeSeries(w http.ResponseWriter, r *http.Request) {
	...
	case "5min":
		...
		SELECT
			...
			COALESCE(AVG(COALESCE(NULLIF(rl.ttft_ms, 0), rl.response_header_ms)) FILTER (WHERE COALESCE(NULLIF(rl.ttft_ms, 0), rl.response_header_ms) > 0 AND rl.status_code > 0 AND rl.status_code < 400), 0) as avg_ttft_ms
		FROM request_logs rl` + vkJoin + `
		WHERE rl.created_at >= $1` + vkFilter + `
		GROUP BY 1
		ORDER BY 1`
