...
func TestTokenUsage_RecordedOnClientDisconnect(t *testing.T) {
	...
	vk, err := vkRepo.Create(ctx, "disconnect-test-key", keyHash, "disco...key", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
...
func TestTokenUsage_RecordedOnNonStreamingSuccess(t *testing.T) {
	...
	vk, err := vkRepo.Create(ctx, "nonstream-test-key", keyHash, "nonst...key", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("failed to create virtual key: %v", err)
	}
	defer func() {
...
func TestTokenUsage_KeyedCompletionChargesTheOwner(t *testing.T) {
	...
	vk, err := vkRepo.Create(ctx, "owner-charge-key", keyHash, "owner...key", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
