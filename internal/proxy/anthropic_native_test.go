...
func TestHandleNativeNonStreaming(t *testing.T) {
	...
	outcome := h.handleNativeNonStreaming(aw, req, st, modelCandidate{}, anthropicNative, resp, 1, 10.0, false)
	aw.Finalize()
...
func TestHandleNativeNonStreaming_JudgesContentNotBytes(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := h.handleNativeNonStreaming(aw, req, st, modelCandidate{}, anthropicNative, resp, 1, 10.0, false); got != outcomeServed {
