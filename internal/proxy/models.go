...
func (h *Handler) ListModels(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		// request that ends that way, rather than a server fault in the access
		// log. debuglog.Error drops the line to Warn for a cancel on its own.
		// Any other failure keeps the 500.
		debuglog.Error("proxy: failed to list models", "error", err)
		writeOpenAIError(w, "failed to list models", cancelStatus(r, err, http.StatusInternalServerError))
		return
	}
	...
	if err != nil {
		...
		writeOpenAIError(w, "failed to list failover groups", cancelStatus(r, err, http.StatusInternalServerError))
		return
	}
