...
func newTestProxyEnvWithUpstream(t *testing.T, upstream *httptest.Server) *testProxyEnv {
	...
	if _, err := virtualKeyRepo.Create(context.Background(), virtualKeyName, keyHash, keyPreview, nil, nil, nil, nil, nil, nil, nil); err != nil {
...
func TestChatCompletions_NonStreaming_Upstream4xxError(t *testing.T) {
	...
	virtualKey, err := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-4xx"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("failed to create virtual key: %v", err)
	}
...
func TestChatCompletions_NonStreaming_Upstream5xxError(t *testing.T) {
	...
	virtualKey, err := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-5xx"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("failed to create virtual key: %v", err)
	}
	defer func() { _ = virtualKeyRepo.Delete(context.Background(), virtualKey.ID) }()
...
func TestChatCompletions_FailoverWithBackoff(t *testing.T) {
	...
	// Create virtual key
	virtualKey, _ := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-failover"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	defer func() { _ = virtualKeyRepo.Delete(context.Background(), virtualKey.ID) }()
...
func TestChatCompletions_HedgingRacesBackup(t *testing.T) {
	...
	virtualKey, _ := virtualKeyRepo.Create(ctx, "test-key", virtualkey.Hash("test-vk-hedge"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
...
func TestChatCompletions_ClientDisconnectDuringBackoff(t *testing.T) {
	...
	virtualKey, _ := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-disconnect"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	defer func() { _ = virtualKeyRepo.Delete(context.Background(), virtualKey.ID) }()
...
func TestChatCompletions_ParamRejectionAutoRetry(t *testing.T) {
	...
	virtualKey, _ := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-retry"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	defer func() { _ = virtualKeyRepo.Delete(context.Background(), virtualKey.ID) }()
...
func TestChatCompletions_NonJSONErrorBody(t *testing.T) {
	...
	virtualKey, err := virtualKeyRepo.Create(context.Background(), "test-key", virtualkey.Hash("test-vk-html"), "sk-tes...", nil, nil, nil, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("failed to create virtual key: %v", err)
	}
	defer func() { _ = virtualKeyRepo.Delete(context.Background(), virtualKey.ID) }()
