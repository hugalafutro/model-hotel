...
-- cost derived from one becomes NULL too, and the price's source label goes
-- with it so the tooltip does not credit a source for a price that is gone.
-- Postgres orders NaN above every number, so NaN is named rather than caught
-- by the comparisons. Idempotent.
UPDATE models SET input_price_per_million = NULL, price_sources = price_sources - 'input'
...
UPDATE models SET output_price_per_million = NULL, price_sources = price_sources - 'output'
...
UPDATE models SET input_price_per_million_cache_hit = NULL, price_sources = price_sources - 'cache_hit'
