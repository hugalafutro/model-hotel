 ...
 * translation key under settings.alerts. The card's pill and the wizard's
 * closing pill classify the same probe, so they classify it once. The card
 * covers a fourth state (nothing configured at all) that cannot happen after
 * the wizard has just configured it, so it stays at the card.
 */
export function statusBadge(status: { reachable: boolean; healthy: boolean }): {
	variant: "ui-badge-danger" | "ui-badge-warn" | "ui-badge-ok";
	key: "statusUnreachable" | "statusUnhealthy" | "statusOk";
} {
	if (!status.reachable)
		return { variant: "ui-badge-danger", key: "statusUnreachable" };
	if (!status.healthy)
		return { variant: "ui-badge-warn", key: "statusUnhealthy" };
	return { variant: "ui-badge-ok", key: "statusOk" };
}
