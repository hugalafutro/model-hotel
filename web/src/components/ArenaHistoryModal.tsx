    ...
    const renderCompetitionDetail = (entry: ArenaHistoryEntry) => {
        ...
        return (
            <div className="mt-3 space-y-3">
                {entry.rounds.map(
                    (round: HistoryBracketRound, rIdx: number) => (
                        <div key={rIdx}>
                            <h5 className="text-xs font-semibold text-(--text-secondary) uppercase tracking-wider mb-1.5">
                                {roundLabel(rIdx, entry.rounds!.length)}
                            </h5>
                            <div className="space-y-1.5">
                                {round.matchups.map(
                                    (mu: HistoryMatchup, mIdx: number) => {
                                        const winnerModelId =
                                            getWinnerFromMatchup(mu);
                                        const aName = mu.slotA
                                            ? shortModelName(mu.slotA.modelId)
                                            : "?";
                                        const bName = mu.slotB
    ...
    const renderCompareDetail = (entry: ArenaHistoryEntry) => {
        ...
        return (
            <div className="mt-3 space-y-2">
                ...
                {responses.map((resp: HistoryResponse, idx: number) => {
