...
describe("VirtualModelTable", () => {
	beforeEach(() => {
		...
		// The pill strips persist their expanded state; each test starts collapsed.
		localStorage.removeItem("modelTable.capPillsCollapsed");
		localStorage.removeItem("modelTable.outputPillsCollapsed");
		mockGetVirtualItems.mockReturnValue([]);
	...
	describe("Filtering", () => {
		...
		it("collapses the filter pills to the first three, the clear button marking a hidden filter", () => {
			setupWithEntries([createModel({ id: "m", capabilities: "{}" })]);
			renderWithProviders(<VirtualModelTable />);

			for (const label of ["Reasoning", "Tools", "Vision"]) {
		...
		it("collapses the output icons to text, image and audio", () => {
			setupWithEntries([createModel({ id: "m" })]);
			renderWithProviders(<VirtualModelTable />);

			for (const label of ["Text out", "Image out", "Audio out"]) {
				expect(screen.getByRole("button", { name: label })).toBeInTheDocument();
			}
		...
		it("greys out unselected pills only once a pill is selected", () => {
			setupWithEntries([createModel({ id: "m", capabilities: "{}" })]);
			renderWithProviders(<VirtualModelTable />);
		...
		it("offers output filter pills even when no loaded row has that output", () => {
			...
			// Output toggles live in their own column, outside the collapsible
			// capability strip, so they need no expand.
		...
		it("sends outputs=text from the text icon", () => {
			setupWithEntries([createModel({ id: "m" })]);
			renderWithProviders(<VirtualModelTable />);
