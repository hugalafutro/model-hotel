...
| `model_prune_days` | int string | `7` | Days a model the provider stopped listing stays in the table as a disabled row before the scheduled discovery pass deletes it. Only rows discovery retired are eligible: models you disabled or enabled by hand, models retired for refusing requests, models of disabled providers, and models that came back (were re-listed) in the last 30 days are never pruned. Runs after each scheduled and startup pass, only for providers that scanned successfully, at most 500 rows per pass. Fleet-synced. `0` keeps every row. | `0`..`180` |
| `log_retention` | duration string | (empty) | How long to keep request and app logs. Any Go duration; the dashboard slider stores whole days as hours. Legacy `1d`/`1w`/`1m` still accepted (`1m` is the 30-day token, not one minute). Empty, `0`, or a zero duration = keep forever; unreadable values are skipped and logged as a warning. Cleanup runs hourly. | `24h`, `48h`, `168h`, `720h`, `1w`, (empty) |
...
| `circuit_breaker_backoff_max` | duration string | `1h` | Ceiling the backed-off cooldown may grow to. A non-positive value does **not** disable backoff - it falls back to 1h. Use `circuit_breaker_backoff_enabled` to turn the feature off. | `10m`, `30m`, `1h`, `6h`, etc. |
...
- **Back Off After Failed Probes** and **Backoff Limit:** Double the cooldown for every half-open probe that fails, up to the limit (default on, limit 60 minutes, range 1-1440). See [Probe backoff](Failover-and-Hotel-Routing#probe-backoff).
