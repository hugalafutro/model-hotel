...
| `model_prune_days` | int string | `30` | Days a model the provider stopped listing stays in the table as a disabled row before the scheduled discovery pass deletes it. Only rows discovery retired are eligible: models you disabled or enabled by hand, models retired for refusing requests, models of disabled providers, and models that flapped in the last 30 days are never pruned. Runs after each scheduled and startup pass, only for providers that scanned successfully, at most 500 rows per pass. Fleet-synced. A row is never pruned in the first 30 days after discovery retired it: that is the window discovery reads flap history from, so a shorter horizon deletes nothing. `0` keeps every row. | `0`, `30`..`365` |
