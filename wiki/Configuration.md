...
| `MODELSDEV_ENABLED` | bool | `true` | Load the models.dev catalog at startup for model enrichment data. A failed load is retried in the background (1, 2, 4, 8, then every 15 minutes) until it succeeds; the next discovery scan after that is enriched. |
...
| `ttft_timeout` | duration string | `1m0s` | Time-to-first-token probe timeout for streaming requests. After the upstream answers 200, the proxy reads ahead to confirm the first token arrives before committing the stream to the client. A token is a frame carrying model output (content, reasoning, a tool call); a role-only opener, a usage-only chunk or a keepalive comment does not count, so a provider that answers 200 and then pings without producing anything is failed over, not waited on. `0s` disables the probe (immediate stream commit). | `0s`, `30s`, `1m0s`, etc. |
| `stream_stall_timeout` | duration string | `30s` | Maximum time without model output during streaming before the connection is terminated and the circuit breaker records a failure. Only frames carrying output (content, reasoning, tool calls) reset the watchdog; keepalive comments and empty deltas keep the connection open but do not count as progress. After 50 output frames the effective timeout is multiplied by 3 to tolerate tool-call pauses and long reasoning chains. `0s` disables the watchdog. | `0s`, `10s`, `30s`, `1m0s`, etc. |
...
| `circuit_breaker_quota_pin_max` | duration string | `24h0m0s` | When a circuit opens because the provider's quota window is spent, pin its cooldown to the provider's real reset deadline instead of `circuit_breaker_cooldown`, so an exhausted provider is not re-probed every minute for the rest of the window. This is the ceiling on how far out a pin may push the cooldown. It also bounds the circuit the quota poller opens by itself when a fresh reading says the account is spent and nothing is holding the provider dark yet, so a restart does not leave an exhausted provider in rotation until real requests fail against it. `0s` switches pinning off (seeding included) and releases a pin already in force, though the change takes up to about 30 seconds to reach the proxy (settings cache TTL). | `0s`, `1h`, `6h`, `24h`, etc. |
...
- **Stream Stall Timeout:** Maximum time without model output during streaming before termination (default `30s`); keepalive comments do not count as output. After 50 output frames the effective timeout is multiplied by 3.
