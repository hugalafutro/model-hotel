...
**Scope:** TOTP guards the admin-token login path, and nothing more. A passkey login and an SSO login (OIDC or GitHub) mint the same admin session without asking for a code: a passkey is a phishing-resistant factor on its own, and an IdP login carries the IdP's own second factor when the IdP enforces one, so a code on top would duplicate it. Configure that factor at the IdP if SSO is your admin login. Sessions minted before TOTP was enabled stay valid too; revoke them from the active-session list if the enablement was a response to a suspected theft. This is by design, not an oversight.
