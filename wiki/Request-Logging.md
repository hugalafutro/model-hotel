...
Every row carries a `cost_usd` column (see the schema above), filled on terminal rows from the serving model's per-token prices at the time the row was written, applied to the usage the provider reported or, when it reported none, to the estimate from the delivered bytes that charges the token limits (the token columns keep the provider's figures). Three places read it:
...
- **In the database**, `error_message` is capped at **10,000 characters**, cut on a character boundary and marked with `…`. An upstream error body is sanitized to the same budget before it gets there. Both then pass through the content fence: an upstream fragment sharing a run of 16 or more characters with the request's own text (a provider quoting the prompt back) is dropped whole rather than partly blanked, leaving the fixed phrase `provider error text withheld` in `error_message` and an empty per-attempt `detail`. The gateway's own wording around it, such as the `all N providers failed; last error: ` wrapper, is never fenced and always survives.
