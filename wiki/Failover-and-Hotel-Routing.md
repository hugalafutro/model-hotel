...
| `modelhotel_provider_quota_reserve_ratio` | `provider_id`, `provider` | The provider's quota reserve as a share (`quota_reserve_percent` / 100); present for a provider with a reserve set and at least one readable window. The breaker pins the provider once a window's used ratio reaches 1 minus this. |
