...
- **A refusal about the account darkens the whole provider at once.** When the 429 body says the account itself has nothing left (OpenAI's `insufficient_quota`, Anthropic's `credit balance is too low`, MiniMax's balance code), the pin is marked account-wide and the provider verdict opens on that single answer, so no sibling model is sent to draw the same refusal. A plan that excludes one model (Z.ai's `no resource package`) stays a per-model pin, and the ordinary span rule still applies to those.
