...
|-----------|----------|--------|
| **HTTP 400** (bad request / param rejection) | **Triggers auto-retry on SAME provider with the offending parameter stripped or renamed** | The provider's 400 names the unsupported parameter; the retry drops it (e.g. an injected `chat_template_args` or `thinking` a strict backend rejects) and tries again on the same provider. The one exception is a param that carries the caller's token budget: when a model demands `max_completion_tokens` instead of `max_tokens` (OpenAI gpt-5 / o-series), the value is renamed rather than dropped, so the response stays capped. The learned rejection is cached per model, so later requests skip the round-trip. A 400 about the response format (DeepSeek's `This response_format type is unavailable now` for `json_schema`, which its API does not serve) is healed differently: the retry goes out in JSON mode (`json_object`) with the schema folded into the leading system message, and that too is learned per model; DeepSeek and Z.AI Coding Plan providers, whose docs list JSON mode alone, get the rewrite on the first attempt. A GLM model on any other provider type keeps `json_schema` on the request but gets the schema in the prompt as well: it takes `json_schema` without error and answers in a shape of its own otherwise. The provider does not validate against the schema, so `strict` is best effort there. A second 400 self-heal covers OpenAI's newest models rejecting tools+reasoning on Chat Completions entirely: the retry re-issues the request against the provider's `/v1/responses` (translated both ways) and the requirement is cached per model, so later tools+reasoning requests route there preemptively. See [API Reference](API-Reference#post-v1chatcompletions). |
...
These behaviours are worth knowing before you reach for these controls:
