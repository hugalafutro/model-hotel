...
3. **Point Front Desk at that topic.** In Front Desk → **Settings → Alerts**, switch on **Send outbound alert notifications** and use the **Phone push via ntfy** helper: enter the server (`https://ntfy.sh`, or your self-hosted server) and the topic from Bellhop (the last path segment of the copied URL), then **Set as target**. This needs an `apprise-api` container in the Front Desk stack; [[Alerting]] has the one-line compose addition and the full pipeline.

Once wired, any Front Desk alert wakes Bellhop within seconds. The push carries no data: Bellhop treats it purely as a wake, runs a fresh fleet poll, and notifies from current Front Desk truth, so it never becomes a second, stale alert source. If you only want plain ntfy notifications without Bellhop's fleet view, skip steps 1 and 2, pick your own secret topic in the Front Desk ntfy helper, and subscribe to it directly in the ntfy app.
