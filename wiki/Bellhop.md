...
3. **Point Front Desk at that topic.** In Front Desk → **Settings → Alerts**, use the **Phone push via ntfy** helper: enter the server (`https://ntfy.sh`, or your self-hosted server) and the topic from Bellhop (the last path segment of the copied URL), then **Set as target**. This needs an `apprise-api` container in the Front Desk stack; [[Alerting]] has the one-line compose addition and the full pipeline.
