...
Once wired, any Front Desk alert wakes Bellhop within seconds. The push carries no data Bellhop trusts: it treats it as a wake, runs a fresh fleet poll that also reads back Front Desk's event log, and notifies from Front Desk's own record, so the notification is the same sentence you would read in the Front Desk event log rather than a stale or reshaped payload. Which events notify is exactly the set switched on under **Alerts**, because those are the events Front Desk pushes for; where one of them is a member going down or recovering, Front Desk's event is used and Bellhop's own health notification for the same outage is not posted, so one outage is one notification. The one payload it does read is Front Desk's test marker, which is not in the event log and would otherwise leave a working pipeline invisible: a **Send test** shows a "push test received" notification on the phone. If you only want plain ntfy notifications without Bellhop's fleet view, skip steps 1 and 2, pick the **Phone (ntfy app)** tile instead, let Front Desk generate the topic, and subscribe to it directly in the ntfy app.
