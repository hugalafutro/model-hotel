...
Once wired, any Front Desk alert wakes Bellhop within seconds. The push carries no data Bellhop trusts: it treats it as a wake, runs a fresh fleet poll that also reads back Front Desk's event log, and notifies from Front Desk's own record, so the notification is the same sentence you would read in the Front Desk event log rather than a stale or reshaped payload. The Front Desk alerts that notify are the ones switched on under **Alerts**, because those are the events Front Desk pushes for; Bellhop's own notifications for a member going down or recovering and for auto-sync drift stay on regardless, and when Front Desk's event for the same outage or drift is being posted, Bellhop's own row for it is not, so one outage is one notification. Bellhop posts at most five Front Desk alerts per check (newest last, so it ends up on top), and the first check after enabling a layer or installing this version only records where the log stands; Bellhop does that check itself when you open the app, so the first push after that is diffed properly. The one payload it does read is Front Desk's test marker, which is not in the event log and would otherwise leave a working pipeline invisible: a **Send test** shows a "push test received" notification on the phone. If you only want plain ntfy notifications without Bellhop's fleet view, skip steps 1 and 2, pick the **Phone (ntfy app)** tile instead, let Front Desk generate the topic, and subscribe to it directly in the ntfy app.
