...
- **Front Desk's role refusals are `forbidden` events.** A paired Bellhop device reaching above its role gets a 403 and the same `auth: insufficient permissions` line the gateway writes, so one rule classifies both. The line names the device id, the role it holds and the role the route wanted, never its token and never its label (a label is caller-set free text, chosen in the public pairing exchange).
- **Two failure kinds are classified but have no scenario on purpose.** SSO callbacks fail for benign reasons (a stale tab, a user pressing deny), and an `insufficient permissions` rejection is an already-authenticated caller hitting a route their role does not cover. Both are visible in the event stream if you want to write your own scenario over them.
