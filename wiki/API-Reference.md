...
**OpenCode Go session header:** OpenCode Go refuses any chat request that carries no `x-opencode-session` (`400 MissingSessionID`), because it pins routing and prompt caching to that id. Requests to a provider of type `opencode-go` therefore always leave the gateway with one, and no other provider type ever sees the header. A client that sends its own `x-opencode-session` has it forwarded as sent after the safety checks, so its conversation keeps one route and one cache; the value is only accepted when it can safely be a header (trimmed, at most 128 bytes, printable ASCII) and does not claim the gateway's own `mh-` namespace, and anything else is treated as absent. Absent, the gateway substitutes a stable opaque id derived from the virtual key (`mh-` plus 32 hex characters), so the same key always maps to the same session while neither the key nor its hash leaves the gateway. Traffic with no virtual key gets a fixed id per surface: admin chat, the retirement probe and the dashboard Test button each have their own. The value is not logged by the proxy.
...
The gateway keeps no conversation state, so every turn re-sends the transcript (what Codex does with `store: false`). Refused with a 400 naming the field, on both modes: `previous_response_id`, `conversation`, `store: true`, `background: true`, `item_reference` and `compaction` input items. Hosted tools other than `web_search` (`file_search`, `code_interpreter`, `image_generation`, `computer`, `mcp`, `shell`, `apply_patch`, `local_shell`), `custom` tools, file ids and audio parts are served only by OpenAI's own endpoint: a request carrying one is accepted when every resolved candidate is OpenAI itself, and refused with a 400 naming the member when any candidate would be translated, since no chat provider can execute them. A `web_search` tool is dropped on translated routes, because Codex advertises one to every provider by default and a provider behind a `hotel/` group simply has no search. Prior `reasoning` items in `input` (encrypted reasoning from OpenAI) are dropped on translated routes: the model reasons fresh from the transcript. Because the mode is chosen per attempt, a `hotel/` request served natively by OpenAI fails over to a translated provider when OpenAI is unavailable. Requests never hedge on this endpoint. Token usage is metered the same way as chat (the native mode reads the Responses `usage` block, cached tokens included) and **request/response content is never logged**.
