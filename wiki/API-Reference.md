...
A `quota_pinned: true` circuit is **not** committed to waiting that long. Every successful quota refresh (each `quota_refresh_interval_min`, 5 minutes by default) lifts the pin from any provider that is no longer out of quota, dropping the circuit back to `circuit_breaker_cooldown`. No event is published when that happens, because nothing transitions: the circuit is still open, only its cooldown changed. Poll `/api/failover-groups/circuit-breaker-status?detail=1` to see it, where `quota_pinned` flips to `false` and `next_retry_at` moves in. See [Failover & Hotel Routing](Failover-and-Hotel-Routing#quota-pinned-cooldowns) for the full behaviour.
