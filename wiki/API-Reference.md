...
`next_retry_at` is the **retry deadline, not the quota reset time**. Under a pin it is the moment the circuit opened plus the pin after it has been clamped to `circuit_breaker_quota_pin_max` and jittered, so on a weekly plan whose quota resets days out it lands at the 24h ceiling instead; under a backoff it is the open moment plus the doubled cooldown. The message of a backed-off `circuit_breaker.open` says so too (`... (backing off after 3 failed retries, next retry in 8m)`), since the message is all an outbound alert renders. It is the same value the circuit-breaker status API publishes under that name, and both derive from one predicate, so the number and the explanation beside it can never disagree.
