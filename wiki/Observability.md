...
- **Overview**: members up (the Up tile), requests per second (Req/s), prompt cache hit share
  over the selected range (Cached: cached prompt tokens over all prompt tokens), error share
  (Errors), p95 latency and TTFT, tokens/s, spend over the selected range (approximate at the
  range edges), providers with a breaker not closed anywhere in the fleet (Breakers), time since
  the most recent member restart (Uptime). Red means look here: only Up, Cached, Errors and
  Breakers carry thresholds. Cached is green from 90% up, orange from 75%, red below.
