...
Every mutating dashboard action (POST, PUT, PATCH, DELETE on the authenticated `/api/*` surface) by any signed-in account, admin or user, is recorded in an audit trail. Two kinds of call are left out because they are not admin actions: machine-to-machine fleet traffic (the heartbeat Front Desk sends every member, and the quota snapshots it relays to them), and a read-only POST that answers a question without changing anything (the backup prune preview). Viewing the trail is admin-only.
