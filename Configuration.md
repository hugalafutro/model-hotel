...
| `DEBUG_LOG_SCOPES` | No | *(empty)* | Comma-separated scope prefixes to enable Debug logging for **only** those areas, when `DEBUG_LOG` is off - e.g. `failover,resolve`. The scope is the text before the first `:` in a log message (matched case-insensitively); see the scope list in [Request Logging](Request-Logging#app-logs). Most App Logs sources emit no Debug records and so cannot be enabled. Lets you turn on Debug for one noisy area without flooding everything at high RPS. Ignored when `DEBUG_LOG=true` (everything is already Debug). |
...
| `circuit_breaker_quota_pin_max` | `24h0m0s` | When a circuit opens because the provider's quota window is spent, pin its cooldown to the provider's real reset deadline instead of retrying every cooldown, up to this ceiling. `0s` switches pinning off, and releases a pin already in force. |
| `circuit_breaker_backoff_max` | `15m0s` | Double an open circuit's cooldown for every half-open retry that fails, up to this ceiling, so a model that stays broken is retried less and less often. A retry that succeeds resets it. `0s` switches backoff off, and releases a backoff already in force; a value at or below the cooldown leaves backoff nothing to add. |
