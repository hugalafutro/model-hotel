...
Passkey login is disabled by default. Enable it by setting `WEBAUTHN_RP_ID` (your domain) in the environment; `WEBAUTHN_RP_ORIGINS` (your origin URLs) falls back to `CORS_ORIGINS`, then to `http://localhost:<port>`. Session tokens are SHA-256 hashed, never stored in plaintext, and expire after 1 day. The lifetime is absolute from login, not extended by activity.
    ...
    services:
        app:
            ...
            environment:
                ...
                # Operator knobs, read from .env (gitignored) so private CIDRs and
                # hostnames never land in the repo. TRUSTED_PROXIES deliberately
                # defaults to empty (trust no forwarded headers): docker's NAT can
                # make ANY direct client appear as the bridge gateway, so shipping
                # a docker-subnet default would let direct clients forge
                # X-Forwarded-For (rate-limit evasion, session-IP spoofing).
